...
public class Player : MonoBehaviour
{
    ...
    [Space(8)] [SerializeField] private float moveSpeed = 5.0f;
    ...
    /// knockback 
    private float knockbackStartTime = 0.0f;

    private bool isKnockback = false;
    private bool isGameInitial = true;
    ...
    private void Start()
    {
        ...
        {
            FyUtility.LogError("transform_ is null.");
        }

        if (this.animator_ == null)
        {
            FyUtility.LogError("animator_ is null.");
        }

        // init
        this.animState = ANIMATION_STATE.Wait;
        this.trigger = new TriggerUtility();
    ...
    public void OnUpdate()
    {
        ...
        this.Move();
        if (Input.GetKeyDown(this.keyConfig.Retrun))
        {
            this.Action();
        }
    }
    ...
    private void Action()
    {
        ...
        if (isInWater && isInFlower)
        {
            Debug.LogWarning("waterとflowerが重なっています");
        }

        if (isInFlower)
        {
            GiftWater(this.trigger.FindGameObjectWithTag(FLOWER_TAG));
        }
        else if (isInWater)
        {
            GetWater();
    ...
    private void Move()
    {
        ...
        var isMove = (isUp || isDown || isLeft || isRight);
        if (!isMove)
        {
            // アニメ
        ...
        this.Translate(moveVec, this.moveSpeed);

        // アニメ
        if (this.animState != ANIMATION_STATE.Run)
        {
            this.ChangeAnimation(ANIMATION_STATE.Run);
        }
    }
    ...
    private void ChangeAnimation(ANIMATION_STATE _animState)
    {
        switch (_animState)
        {
            ...
            case ANIMATION_STATE.Knockback:
                this.animator_.SetBool("isRun", false);
                break;
        }
    ...
    private void UpdateStatus()
    {
        ...
        if (this.isKnockback)
        {
            ...
            if (!this.isKnockback)
            {
                this.knockbackStartTime = float.MaxValue;
            }
        }
        else
        {
